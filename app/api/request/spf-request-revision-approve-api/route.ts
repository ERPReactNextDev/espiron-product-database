...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    if (revisionError || !revisionData) {
      return NextResponse.json({ message: "No ongoing revision found" }, { status: 404 });
    }

    const {
      id,
      spf_revision_approval_sales_status,
      spf_revision_approval_sales_date,
      date_created,
      date_updated,
      status,
      ...requestData
    ...
    const { error: requestError } = await supabase
      .from("spf_request")
      .update({ ...requestData, date_updated: new Date().toISOString() })
      .eq("spf_number", spf_number);
    ...
    return NextResponse.json({ success: true, message: "Revision approved and applied" });
  } catch (err: any) {
