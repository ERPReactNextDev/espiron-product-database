...
export async function POST(req: NextRequest) {
  try {
    ...
    /* ΓöÇΓöÇ Fetch spf_request metadata ΓöÇΓöÇ */
    ...
    if (spfRequestError || !spfRequest) {
      ...
      return NextResponse.json({ message: "SPF request not found" }, { status: 400 });
    }
    ...
    /* ΓöÇΓöÇ Resolve item_added_author ΓöÇΓöÇ */
    ...
    /* ΓöÇΓöÇ Pre-fetch all unique supplier contacts ΓöÇΓöÇ */
    ...
    /* ΓöÇΓöÇ Group products by row ΓöÇΓöÇ */
    ...
    /* ΓöÇΓöÇ Per-row accumulators ΓöÇΓöÇ */
...
// price_validity: store as-is (text column, delimited string)
        ...
        /* ΓöÇΓöÇ Original tech specs (for editing later) ΓöÇΓöÇ */
        ...
        /* ΓöÇΓöÇ Product Reference ID for syncing ΓöÇΓöÇ */
        productRefIDs.push(p?.productReferenceID || p?.id || "-");

        /* ΓöÇΓöÇ Branch/Country selection ΓöÇΓöÇ */
    ...
    const finalSupplierModelCodes = rowSupplierModelCodes.join(ROW_BOUNDARY);
    ...
    // ΓöÇΓöÇ TDS PDF URLs per product per row ΓöÇΓöÇ
    ...
    /* ΓöÇΓöÇ Check existing SPF ΓöÇΓöÇ */
    ...
    if (checkError) {
      ...
      return NextResponse.json(checkError, { status: 500 });
    }
    ...
    if (!existing) {
      ...
      /* ΓöÇΓöÇ Version history v1 ΓöÇΓöÇ */
      ...
      /* ΓöÇΓöÇ UPDATE parent spf_request date_updated ΓöÇΓöÇ */
      await supabase
        .from("spf_request")
    ...
    /* ΓöÇΓöÇ Delete draft from spf_creation_draft ΓöÇΓöÇ */
    try {
    ...
    return NextResponse.json({ success: true, message: "SPF created successfully" }, { status: 200 });
