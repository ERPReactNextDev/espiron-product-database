...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    /* ΓöÇΓöÇ Resolve draft_author ΓöÇΓöÇ */
      ...
      for (let optIdx = 0; optIdx < rowProducts.length; optIdx++) {
        ...
        if (
          ...
          if (typeof packagingData === "string" && packagingData.trim() && packagingData.trim() !== "-") {
            // Already a formatted string (e.g. "23 cm x 23 cm x 23 cm") ΓÇö use as-is
        ...
        /* ΓöÇΓöÇ Original tech specs ΓöÇΓöÇ */
        ...
        /* ΓöÇΓöÇ Is Existing (for tracking products from original SPF vs newly added) ΓöÇΓöÇ */
        isExisting.push(p?.__isExisting === true ? "true" : "false");

        /* ΓöÇΓöÇ MOQ, Quotations Validity, Production Lead Time, Delivery Lead Time ΓöÇΓöÇ */
    ...
    if (upsertError) {
      ...
      return NextResponse.json({ message: "Failed to save draft", error: upsertError }, { status: 500 });
    }
