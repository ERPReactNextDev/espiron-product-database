...
const buildFingerprint = (
  ...
  [
    ...
    normalizeSupplierBrand(supplierBrand), normalizeField(supplierModelCode),
...
function buildCommercialDetails(row: ParsedProductRow): Record<string, any> {
  ...
  const base = {
    ...
    supplierModelCode: row.supplierModelCode || "",
  ...
  const checkForDuplicates = async (rows: ParsedProductRow[]): Promise<{
    ...
    snap.docs.forEach((d) => {
      ...
      existingFingerprints.add(buildFingerprint(
        ...
        data.commercialDetails?.supplierModelCode ?? "",
    ...
    for (const row of rows) {
      ...
      const fp = buildFingerprint(row.usage, row.family, row.productClass, row.pricePoint, row.brandOrigin, row.supplierBrand, row.supplierModelCode);
